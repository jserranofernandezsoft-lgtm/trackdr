...
@media print{.no-print{display:none!important;}body{padding:10px;}.qr-card{box-shadow:none;border:1.5px solid #cbd5e1;}}
</style>
</head>
<body>
<div class="no-print">
    <button onclick="window.print()" class="btn btn-cyan">🖨️ Imprimir QR</button>
    <button onclick="window.close()" class="btn btn-gray">✕ Cerrar</button>
    <span style="font-size:12px;color:#94a3b8;align-self:center;">Imprime y pega en el accesorio</span>
</div>
<div class="qr-card">
    <div class="ms-grid">
    ...
    <div class="brand-sub">Activo Menor</div>
    ...
    @if($activo->serial)<div class="qr-serial">S/N: {{ $activo->serial }}</div>@endif
    @if($activo->departamento)<div class="qr-serial" style="margin-top:3px;">{{ $activo->departamento->nombre }}</div>@endif
    <div class="divider"></div>
