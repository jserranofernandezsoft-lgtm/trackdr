...
<div class="no-print" style="margin-bottom:20px;display:flex;gap:10px;align-items:center;">
    <button onclick="window.print()"
        style="background:#0891b2;color:#fff;border:none;padding:9px 20px;border-radius:6px;font-size:13px;font-weight:600;cursor:pointer;">
...
<div class="filtros-bar">
    ...
    @if(!empty($filtros['estado']))<span><strong>Estado:</strong> {{ \App\Models\ActivoMenor::$estados[$filtros['estado']] ?? $filtros['estado'] }}</span>@endif
    @if(!empty($filtros['tipo']))<span><strong>Tipo:</strong> {{ \App\Models\ActivoMenor::$tipos[$filtros['tipo']] ?? $filtros['tipo'] }}</span>@endif
</div>
@endif
...
@php
    ...
    $baja  = $activos->where('estado','baja')->count();
@endphp
<div class="stats-row">
    ...
    <div class="stat-box red">   <div class="num">{{ $baja }}</div><div class="lbl">Dados de Baja</div></div>
</div>
...
<table>
    ...
    <tr>
        ...
        <td class="fw">{{ trim(($a->marca ?? '') . ' ' . ($a->modelo ?? '')) ?: '—' }}</td>
        ...
        <td>
            @php $badge = ['disponible'=>'badge-green','asignado'=>'badge-purple','baja'=>'badge-red','bodega'=>'badge-gray'][$a->estado] ?? 'badge-gray' @endphp
    ...
    @empty
    <tr><td colspan="9" style="text-align:center;padding:24px;color:#94a3b8;">No hay activos menores registrados</td></tr>
    @endforelse
    </tbody>
</table>
